...
// Платная фича: при наличии кредитов — списать 1 кр.; иначе 402 → pay /api/paid/insight/weekly.
...
export async function GET(req: NextRequest) {
    console.log('[API] GET /api/insight/weekly called');
    try {
        ...
        const cached = await getAICache<any>(supa, userId, { endpoint: 'insight/weekly', input: cacheKey, cacheHours: 24 });
        if (cached) {
            await logAIRequest(supa, userId, userPlan, 'insight/weekly', { skipConsume: true, usedBonusCredit: false });
        ...
        console.error('[Insight Weekly] Error:', e?.message, e?.stack);
