# Система монетизации и оплаты

## 📋 Общая информация

Этот документ описывает **текущую систему монетизации** приложения (обновлено: 2025-01-11).

---

## 💰 Текущая система монетизации (2025-01-11)

### 🆓 Бесплатные лимиты для всех пользователей

| Функция | Лимит | Описание |
|---------|-------|----------|
| **Привычки** | 5 | Максимум 5 активных привычек |
| **Цели** | 3 | Максимум 3 активные цели |
| **AI запросы** | 5/день | 5 бесплатных AI запросов в день (Chat, Insights, Reviews) |
| **Wheel of Life** | ♾️ | Безлимитно |
| **Стрики и XP** | ♾️ | Безлимитно |
| **Badges** | 10 типов | Все 10 бейджей доступны бесплатно |

---

## 💎 Пакеты AI кредитов

**Цена за кредит:** $0.29 (базовый пакет)

| Пакет | Кредитов | Цена USD | Цена за кредит | Срок действия | Скидка |
|-------|----------|----------|----------------|---------------|--------|
| **small** | 6 | $1.74 | $0.29 | 31 день | - |
| **medium** | 15 | $3.49 | $0.23 | 62 дня | ~20% |
| **large** | 40 | $7.99 | $0.20 | 93 дня | ~31% |

**API Endpoint:** `/api/paid/credits/[pack]`  
**Метод:** `POST`  
**Оплата:** x402 (Base network)

**Как работает:**
1. Пользователь покупает пакет кредитов через x402
2. Кредиты начисляются в таблицу `user_credits`
3. При использовании AI запроса сверх лимита (5/день) списывается 1 кредит
4. Кредиты имеют срок действия (expires_at)

**Файл конфигурации:** `src/lib/pricing.ts` → `CREDIT_PACKS`
...
| Тип | Цена USD | Описание | Что даёт |
|-----|----------|----------|----------|
| **habits** | $2.99 | Unlimited Habits | Убирает лимит 5 привычек навсегда |
| **goals** | $2.99 | Unlimited Goals | Убирает лимит 3 цели навсегда |
| **bundle** | $4.99 | Full Unlock | Оба unlock (экономия $0.99) |

**API Endpoint:** `/api/paid/unlock/[type]`  
**Метод:** `POST`  
**Оплата:** x402 (Base network)

**Как работает:**
1. Пользователь покупает unlock через x402
2. Запись добавляется в таблицу `user_unlocks`
3. Лимит проверяется через `src/lib/featureLimits.ts`
4. Разблокировка действует навсегда (нет срока действия)

**Файл конфигурации:** `src/lib/pricing.ts` → `UNLOCKS`  
**SQL таблица:** `user_unlocks` (см. `sql/add_user_unlocks.sql`)

---

## 🎫 NFT Badge Minting

| Функция | Цена USD | Описание |
|---------|----------|----------|
| **Mint Badge** | $0.19 | Минт NFT бейджа на Base (Zora 1155) |

**API Endpoint:** `/api/mints/mint`  
**Метод:** `POST`  
**Оплата:** x402 (Base network)

---

## 📊 Структура цен (актуальная)

**Файл:** `src/lib/pricing.ts`

```typescript
export const CREDIT_PACKS = {
    small: { credits: 6, priceUsd: 1.74, ttlDays: 31 },
    medium: { credits: 15, priceUsd: 3.49, ttlDays: 62 },
    large: { credits: 40, priceUsd: 7.99, ttlDays: 93 },
};

export const UNLOCKS = {
    habits: { priceUsd: 2.99, name: 'Unlimited Habits' },
    goals: { priceUsd: 2.99, name: 'Unlimited Goals' },
    bundle: { priceUsd: 4.99, name: 'Full Unlock' },
};

export const PRICES_USD = {
    "/api/paid/credits/small": 1.74,
    "/api/paid/credits/medium": 3.49,
    "/api/paid/credits/large": 7.99,
    "/api/paid/unlock/habits": 2.99,
    "/api/paid/unlock/goals": 2.99,
    "/api/paid/unlock/bundle": 4.99,
    "/api/mint": 0.19,
};
```
...
## 🔄 Логика работы AI кредитов

1. **Бесплатные запросы:** Первые 5 AI запросов в день бесплатны
2. **Бонусные кредиты:** Если лимит исчерпан, используются кредиты из `user_credits`
3. **Списание:** При запросе сверх лимита автоматически списывается 1 кредит
4. **Проверка:** `src/lib/aiLimits.ts` → `checkAILimit()` и `logAIRequest()`

**Пример:**
- Пользователь сделал 5 запросов (бесплатно)
- Делает 6-й запрос → списывается 1 кредит из `user_credits`
- Если кредитов нет → ошибка 402 (Payment Required)

---

## 📝 Устаревшая информация (ниже)

> **Внимание:** Информация ниже относится к старой системе (Pro/Premium подписки), которая была упрощена. Оставлена для справки.

---

## 💰 Структура цен (устаревшая)
