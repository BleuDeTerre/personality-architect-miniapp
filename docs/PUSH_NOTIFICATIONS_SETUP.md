...
> ℹ️ **Примечание.** На данный момент Web Push отключены, вместо них cron `/api/notifications/cron` отправляет уведомления через Neynar `publishFrameNotifications`. Данные ниже пригодятся, если потребуется вернуть web push.
